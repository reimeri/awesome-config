...
local focus_switch_byindex = function(dir)
	return function()
...
function hotkeys:init(args)
	...
	local menu_keys_move = {
		...
		{
			{ }, back, redflat.menu.action.back,
			{ description = "Go back", group = "Navigation" }
		},
		{
			{ }, enter, redflat.menu.action.enter,
	...
	self.raw.root = {
		...
		{
			{ env.mod }, "j", focus_switch_byindex(1),
			{ description = "Rotate the stack +", group = "Client focus" }
		},
		{
			{ env.mod }, "k", focus_switch_byindex(-1),
			{ description = "Rotate the stack -", group = "Client focus" }
		},
		{
	...
	self.raw.client = {
		...
		{
			{ env.mod, }, "t", function(c) c.ontop = not c.ontop end,
